...
object SFOFireServiceCallAnalysisDFSQL {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder().config(conf = conf).master("local").appName("SFOFireServiceCallAnalysisDFSQL").getOrCreate()
...
//      .csv(args(0))
      val fireServiceCallRawDF = spark.read.format("csv").option("header", "true").schema(fireServiceCallSchema)
    .csv("C:\\Anil\\Learning\\Fire_Department_Calls_for_Service.csv")
