...
object FireServiceCallAnalysis {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder().config(conf = conf).master("local").appName("FireServiceCallAnalysis").getOrCreate()

    // LOAD DATA
    //val fireServiceCallRawRDD = spark.sparkContext.textFile(args(0))
    val fireServiceCallRawRDD = spark.sparkContext.textFile("C:\\Anil\\Learning\\Fire_Department_Calls_for_Service.csv")
